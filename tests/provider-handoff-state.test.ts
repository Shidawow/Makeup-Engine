...
describe('provider handoff state', () => {
  it('is valid JSON with required provider handoff fields', async () => {
    ...
    expect(handoff.currentTask).toContain('9H');
    expect(handoff.taskStatus).toBeTruthy();
    expect(handoff.lastCompletedPhase).toBe('9H');
    expect(handoff.nextRecommendedPhase).toBe('9I');
  ...
  it('records source-of-truth and guardrail notes', async () => {
    ...
    expect(notes).toContain('Phase 9I should review anonymous internal trial evidence only');
