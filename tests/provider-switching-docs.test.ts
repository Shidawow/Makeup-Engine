...
describe('provider switching docs', () => {
  ...
  it('provides phase 14B to 14C switch templates with historical 14A, 13D, 13C, 13B, 13A, 12E, 12D, 12C, 12B, 12A, 11D, 11C, 11B, 11A, 10U, and older markers', async () => {
    ...
    expect(prompt).toContain('step guidance trust wording');
    expect(prompt).toContain('operator workflow explanation');
    expect(prompt).toContain('ordinary User App paths free of sprint/roadmap/admin terms');
    expect(prompt).toContain('Historical handoff marker retained for Phase 13C recovery tests');
    ...
    expect(prompt).toContain('lastCompletedPhase: 13A');
    expect(prompt).toContain('nextRecommendedPhase: 13B');
    expect(prompt).toContain('MVP Trial Content Pack & Founder Demo Review');
    expect(prompt).toContain('Founder Demo Review');
    ...
    expect(prompt).toContain('nextRecommendedPhase: 12D');
    expect(prompt).toContain('Photo-to-Template Operator Workflow & Draft Preview QA');
    expect(prompt).toContain('Photo-to-Template Draft Integration & Human Review Editing');
    expect(prompt).toContain('semantic candidate binding matrix');
    expect(prompt).toContain('source type / confidence band / evidence / limitations preserved');
    ...
    expect(prompt).toContain('Phase 11C completed');
    ...
    expect(prompt).toContain('Historical handoff marker retained for Phase 10S recovery tests');
