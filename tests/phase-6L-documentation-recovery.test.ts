...
describe('Phase 6L documentation recovery', () => {
  it('keeps 6L prototype docs while current recovery state has advanced to 7A', () => {
    ...
    expect(readText('docs/prompts/MASTER_CODEX_CONTEXT.md')).toContain('Phase 7G completed');
    ...
    expect(snapshot.lastCompletedPhase).toBe('7G');
    expect(snapshot.nextRecommendedPhase).toBe('7H');
    expect(snapshot.nextAction).toContain('Phase 7H');
    ...
    expect(latestHandoff.fromPhase).toBe('7G');
    expect(latestHandoff.toPhase).toBe('7H');
    expect(latestHandoff.nextAction).toContain('Phase 7H');

    const providerHandoff = readJson<ProviderHandoff>('project-state/provider-handoff.json');
    expect(providerHandoff.lastCompletedPhase).toBe('7G');
    expect(providerHandoff.nextRecommendedPhase).toBe('7H');
    ...
    expect(guardrails.phase).toBe('7G');
