...
- Last completed phase: `Phase 13B`.
- Current completed business phase: `Phase 13B - Founder Trial Feedback Capture & MVP Gap Prioritization`.
- Next recommended phase: `Phase 13C - MVP Gap Resolution Sprint Planning`.
- Strategic focus: registry chain paused after Phase 10U; active work is founder/internal feedback capture, MVP gap prioritization, and planning the next demo-quality sprint.
...
- `docs/phases/phase-11B.md`

Phase 11B polishes the ordinary-user preparation, step-by-step guidance,
mobile touch actions, and completion summary. The registry chain remains paused
after Phase 10U, Phase 10V is still not the active next phase, and the shell
...
Phase 11D packages the ordinary-user User App MVP shell for operator-led demo
and QA. It documents the demo path, user path checks, mobile checks, forbidden
terms checks, known limitations, Vision Analysis notes, Template Studio notes,
and Git hygiene checks. The registry chain remains paused after Phase 10U,
Phase 10V is still not the active next phase, and the shell still does not
write registries, publish, create production writers, replace the current User
App Shell package, add backend/camera/AR/API scope, or train models. Next
recommended phase: `Phase 12A - Photo-to-Template Draft Reality Check`.

## Phase 12A Addendum

- `docs/product/photo-to-template-draft-reality-check.md`
- `docs/product/photo-to-template-field-source-matrix.md`
- `docs/phases/phase-12A.md`

Phase 12A audits the real photo-to-template draft capability. It classifies
fields as real_from_photo, facemesh_derived, region_qa_derived,
pixel_rule_derived, semantic_rule_derived, template_rule_derived,
demo_fixture, placeholder, human_required, or unsupported. The current system
supports semi-automatic template draft generation with human review, but it
