    ...
    <section className="grid gap-4">
      ...
      <section className="rounded-lg border border-stone-200 bg-white p-4 shadow-soft">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase text-teal-700">Phase 7H readiness</p>
            <h2 className="text-lg font-semibold text-stone-950">App 就绪度检查</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
              这里从模板包、步骤指导、本地引导、偏好、发现、会话、隐私和移动端交互做本地门禁判断。
              它只用于产品和 QA 复核，不会创建生产 App、账号、后端、相机、AR 或训练数据。
            </p>
          </div>
          ...
          {report.checks.map((item) => (
            ...
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                ...
                <span
                  className={`w-fit rounded-md border px-2 py-1 text-xs ${checkStatusClass[item.status]}`}
                >
