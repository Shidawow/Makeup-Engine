      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
            ...
            当前是确定性清单检查，并由 Phase 7H 的本地 HTTP smoke 和 SSR 测试补充。
          </p>
        </div>
          ...
          <article className="rounded-md border border-stone-200 bg-stone-50 p-3" key={item.checkId}>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              <span className={`w-fit rounded-md border px-2 py-1 text-xs ${statusClass[item.status]}`}>
                {item.status === 'passed' ? '通过' : item.status === 'warning' ? '提醒' : '阻断'}
              </span>
            </div>
            <p className="mt-2 text-sm text-stone-600">{item.evidence[0]}</p>
            <p className="mt-1 text-xs text-stone-500">建议：{item.recommendation}</p>
          </article>
        ))}
      </div>
    </section>
