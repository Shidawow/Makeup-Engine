...
export function FaceMeshMakeupIntelligencePanel({
  ...
  if (!regionQa || !attributeCandidates || !stepSequence || !templateDraft) {
    ...
    return (
      <section className="rounded-lg border border-stone-200 bg-white p-4 shadow-soft">
        ...
        <div className="mt-3 grid gap-2 rounded-md border border-cyan-200 bg-cyan-50 p-3 text-sm text-cyan-900 md:grid-cols-4">
          <div>
            <p className="font-semibold">真实写入执行授权</p>
            <p className="mt-1 text-xs leading-5">
              Execution Authorization blocked：缺少 10P final review gate ready。
            </p>
          </div>
          <div>
            <p className="font-semibold">老板授权范围</p>
            <p className="mt-1 text-xs leading-5">
              只授权进入 10Q 授权阶段；不授权真实写入 registry、不授权发布、不授权替换当前用户 App 包、不授权创建 production writer。
            </p>
          </div>
          <div>
            <p className="font-semibold">Authorization Checklist</p>
            <p className="mt-1 text-xs leading-5">
              复核 dry-run only、no actual write、no publish、no package replacement、no production writer。
            </p>
          </div>
          <div>
            <p className="font-semibold">Authorization Handoff</p>
            <p className="mt-1 text-xs leading-5">
              只可进入未来真实写入执行计划；不是实际写入。
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 grid gap-2 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900 md:grid-cols-5">
          <div>
            <p className="font-semibold">受保护模拟器复核闸门</p>
            <p className="mt-1 text-xs leading-5">
              Review Gate blocked：缺少 10S simulation validation ready。
            </p>
          </div>
          <div>
            <p className="font-semibold">Review Boundary</p>
            <p className="mt-1 text-xs leading-5">
              复核闸门，不是实际写入；仍然 dry-run only。
            </p>
          </div>
          <div>
            <p className="font-semibold">No Registry Mutation</p>
            <p className="mt-1 text-xs leading-5">
              不授权真实写入 registry，不 mutation registry，不发布。
            </p>
          </div>
          <div>
            <p className="font-semibold">Review Checklist</p>
            <p className="mt-1 text-xs leading-5">
              simulated preflight review / simulated write lock review / simulated write operation review / simulated audit events review / simulated rollback review / simulated failure handling review。
            </p>
          </div>
          <div>
            <p className="font-semibold">Review Handoff</p>
            <p className="mt-1 text-xs leading-5">
              只可进入未来真实写入批准边界；不替换当前用户 App 包，不创建 production writer。
            </p>
          </div>
        </div>
  ...
  const realWriteExecutionAuthorization =
    createRealWriteExecutionAuthorization({
      finalReviewGate: finalRealWriteReviewGate,
    });
  const realWriteExecutionAuthorizationChecklist =
    createRealWriteExecutionAuthorizationChecklist({
      finalReviewGate: finalRealWriteReviewGate,
    });
  const realWriteExecutionAuthorizationHandoff =
    createRealWriteExecutionAuthorizationHandoff({
  ...
  const guardedSimulatorReviewGate = createGuardedSimulatorReviewGate({
    simulator: guardedRealWriteExecutionSimulator,
    validation: guardedRealWriteExecutionSimulatorValidation,
  });
  const guardedSimulatorReviewChecklist =
    createGuardedSimulatorReviewChecklist({
      gate: guardedSimulatorReviewGate,
    });
  const guardedSimulatorReviewHandoff = createGuardedSimulatorReviewHandoff({
    gate: guardedSimulatorReviewGate,
